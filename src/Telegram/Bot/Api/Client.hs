...
class (Monad m) => TelegramClient m where
  ...
  editMessageText :: EditMessageRequest -> m ()
  editMessageReplyMarkup :: EditMessageReplyMarkupRequest -> m ()
  sendPhoto :: SendPhotoRequest -> m ()
  answerInlineQuery :: AnswerInlineQueryRequest -> m ()

instance MonadIO m => TelegramClient (AppM a Text m) where
  getUpdate = do
    AppCtx {..} <- ask
    ...
    let opts =
          defaults
            ...
            & param "limit" .~ ["1"]
            & param "timeout" .~ ["30"]
            & param "allowed_updates" .~ ["[\"message\", \"inline_query\"]"]
            & manager
              .~ Left
                (tlsManagerSettings
  ...
  editMessageText EditMessageRequest {..} = do
    TelegramConfig tgBaseUrl (Token token) _ <- asks $ telegramCfg . config
  ...
  answerInlineQuery AnswerInlineQueryRequest{..} = do
    TelegramConfig tgBaseUrl (Token token) _ <- asks $ telegramCfg . config
    let partInlineQueryId = partText "inline_query_id" aiqInlineQueryId
        partX = partText "results" . decodeUtf8 . BL.toStrict . encode $ aiqResults
    void $ liftIO $ post (T.unpack (T.concat [tgBaseUrl, token, "/answerInlineQuery"])) [partInlineQueryId, partX]
